...
def readCommand(argv):
    ...
    if options.gameToReplay != None:
        ...
        gamefiles = [options.gameToReplay+'/'+f for f in listdir(options.gameToReplay) if isfile(join(options.gameToReplay, f))]

        import pickle
        for game in gamefiles[-5:]:
            f = open(game, 'rb')
            try:
                recorded = pickle.load(f, encoding='bytes')
            finally:
                f.close()
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    ...
    for i in range(numGames):
        ...
        if (record is True) and (i % 200 == 0):
            import time
            import pickle
            fname = ('./recorded/recorded-game-%d' % (i + 1)) + \
                    '-'.join([str(t) for t in time.localtime()[1:6]]) + '.pkl'
            components = {'layout': layout, 'actions': game.moveHistory}

            # Convert the dictionary to bytes using pickle.dumps()
